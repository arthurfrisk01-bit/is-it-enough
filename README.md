...
不是你不够自律——这些应用本来就设计得让人停不下来。「够了吗」不跟你讲道理，只做一件事：在你刷过头的时候，轻轻把你叫醒，再给你一个体面停下来的台阶。

## 它怎么帮你

- **刷久了会提醒你**：在同一个应用上连续刷到设定时长（默认 10 分钟，可选 5 / 10 / 15 / 20），它就会出现。
...
- **不用你操心**：自动识别你正在用哪个应用，切走再切回来继续算；它自己不算数，桌面也不算。
...
| **iOS** | 极简版：受系统限制不做后台监听，由用户主动开启专注计时与呼吸引导 |
